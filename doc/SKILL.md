...
## Path Rule
- Treat `output/doc` as a repo-local path relative to the current working directory.
- Resolve it to `<current-repo>/output/doc`.
- Prefer repo-local output over user-home or skill-package-relative directories unless the user explicitly asks for another location.
...
- Use `output/doc/tmp/` for intermediate files; delete when done.
